...
#     """
#     astro dict ko advanced/batch engines ke liye normalize karta hai.

#     IMPORTANT:
#     - D1 data preserve rahega
#     - D9 preserve rahega
#     - Future Vargas (D3/D7/D10 etc.) bhi preserve rahenge
#     - La (Lagna) explicitly pass hoga
#     """
...
#     for our_code in all_codes:

#         if our_code not in astro:
#             continue

#         p = astro.get(our_code) or {}

#         # ─────────────────────────────────────────
#         # D1 sign
#         # ─────────────────────────────────────────
#         vargas = p.get("Vargas") or {}

#         d1_raw = vargas.get("D1")

#         if isinstance(d1_raw, dict):
#             sign_idx = d1_raw.get("Idx")
#         elif isinstance(d1_raw, (int, float)):
#             sign_idx = int(d1_raw)
#         else:
#             sign_idx = None
...
#         # Basic data
#         # ─────────────────────────────────────────
#         full_deg = p.get("Degree", 0) or 0

#         try:
#             full_deg = float(full_deg)
...
#         #   D3 / D7 / D10 / etc.
#         #
#         # jo astro mein hai, woh intact pass hoga.
#         # ─────────────────────────────────────────
#         preserved_vargas = dict(vargas)

#         retrograde = bool(
#             p.get("Retro", False)
#             or p.get("retrograde", False)
#             or p.get("Retrograde", False)
#         )
...
#             "full_degree": round(full_deg, 4),
#             "dignity": dignity,
#             "sign": p.get("Rashi", ""),

#             # Compatibility: existing engines may use either spelling
#             "retrograde": retrograde,
#             "Retrograde": retrograde,

#             # CRITICAL:
#             # D1 + D9 + future Vargas preserved
#             "Vargas": preserved_vargas,
#         }
...
def _build_planets_for_batch_engines(astro: Dict, planet_house_map: Dict) -> Dict:
    """
    api.py ke already-calculated ``astro`` ko engines ke common format mein
    ek baar normalize karta hai.

    IMPORTANT:
    - D1/D3/D7/D9/D10/... ko yahan dobara calculate nahi kiya jaata.
    - api.py se aaye raw ``Vargas`` ko preserve kiya jaata hai.
    - Exact/full planetary degree bhi preserve hota hai.
    - La (Lagna) bhi downstream engines ko milta hai.

    Is function ka kaam sirf data adapter hai; frontend tab change hone par
    koi refresh/recalculation nahi hota.
    """
    result = {}
    ...
    for our_code in all_codes:
        ...
        # D1 primary sign index. Different chart builders may store it as
        # either {"Idx": n} or directly as an integer.
        d1_raw = vargas.get("D1")
        if isinstance(d1_raw, dict):
            sign_idx = d1_raw.get("Idx")
        elif isinstance(d1_raw, (int, float)):
        ...
        # D1 sign should agree with the exact full longitude. If the API
        # D1 index is missing/inconsistent, derive the 0-based sign from
        # the full longitude as a safe fallback. Raw Vargas remain untouched.
        try:
            degree_sign_idx = int(full_deg / 30.0) % 12
        except (TypeError, ValueError):
            degree_sign_idx = None

        try:
            sign_idx = int(sign_idx) % 12 if sign_idx is not None else None
        except (TypeError, ValueError):
            sign_idx = None

        if sign_idx is None:
            sign_idx = degree_sign_idx
        dignity = p.get("Dignity", "")

        # Lagna planet_house_map mein nahi hota; uska house 1 hai.
        house = 1 if our_code == "La" else planet_house_map.get(our_code, 0)

        # IMPORTANT: saare available D-charts raw form mein preserve.
        preserved_vargas = dict(vargas)
